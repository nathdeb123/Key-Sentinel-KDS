...
class NetworkSecurityMonitor:
    ...
    def setup_ui(self):
        ...
        self.sniff_btn = tk.Button(control_frame, text="📡 Start Sniffing", 
                 font=("Helvetica", 11), bg="#6600aa", fg="white",
                 command=self.toggle_sniffing, width=15)
        self.sniff_btn.pack(side="left", padx=5, pady=5)
    ...
    def setup_analysis_tab(self):
        ...
        # Packet analysis
        packet_frame = tk.LabelFrame(analysis_frame, text="Packet Sniffing Analysis", 
                                    font=("Helvetica", 12, "bold"),
                                    fg="#00ff41", bg="#1a1a1a", bd=2)
        packet_frame.pack(fill="both", expand=True, padx=10, pady=5)
        
        # Packet table
        packet_columns = ("Timestamp", "Source IP", "Dest IP", "Protocol", "Port", "Flags", "Risk")
        self.packet_tree = ttk.Treeview(packet_frame, columns=packet_columns, show="headings")
        
        for col in packet_columns:
            self.packet_tree.heading(col, text=col)
            self.packet_tree.column(col, width=90)
        
        packet_v_scroll = tk.Scrollbar(packet_frame, orient="vertical", command=self.packet_tree.yview)
        self.packet_tree.configure(yscrollcommand=packet_v_scroll.set)
        
        self.packet_tree.pack(side="left", fill="both", expand=True, padx=5, pady=5)
        packet_v_scroll.pack(side="right", fill="y")
    ...
    def toggle_sniffing(self):
        """Toggle network packet sniffing"""
        if not SCAPY_AVAILABLE:
            messagebox.showerror("Scapy Not Available", 
        ...
        if not self.sniffing:
            self.sniffing = True
            self.sniff_btn.config(text="📡 Stop Sniffing", bg="#aa0000")
            self.log_suspicious("Network packet sniffing started")
            
            # Start sniffing thread
            self.sniff_thread = threading.Thread(target=self.start_packet_sniffing, daemon=True)
            self.sniff_thread.start()
        else:
            self.sniffing = False
            self.sniff_btn.config(text="📡 Start Sniffing", bg="#6600aa")
            self.log_suspicious("Network packet sniffing stopped")

    def start_packet_sniffing(self):
        """Start capturing and analyzing network packets"""
        if not SCAPY_AVAILABLE:
            self.log_suspicious("Scapy not available for sniffing")
            return
        
        try:
            self.log_suspicious("Initializing packet sniffer...")
            # Sniff packets and process them
            sniff(prn=self.process_packet, store=False, stop_filter=lambda x: not self.sniffing)
        except PermissionError:
            self.window.after(0, lambda: messagebox.showerror("Permission Denied", 
                "Packet sniffing requires administrator/root privileges"))
            self.sniffing = False
            self.window.after(0, lambda: self.sniff_btn.config(text="📡 Start Sniffing", bg="#6600aa"))
        except Exception as e:
            self.log_suspicious(f"Sniffing error: {str(e)}")
            self.sniffing = False
            self.window.after(0, lambda: self.sniff_btn.config(text="📡 Start Sniffing", bg="#6600aa"))
    ...
    def process_packet(self, packet):
        ...
        try:
            ...
            if IP in packet:
                ...
                if dest_ip in self.blocked_ips or source_ip in self.blocked_ips:
                    risk = "High"
                ...
                if TCP in packet:
                    ...
                    # Check for suspicious ports
                    if port in self.suspicious_ports or tcp_layer.sport in self.suspicious_ports:
                        risk = "High"
                    ...
                    # Check for suspicious ports
                    if port in self.suspicious_ports or udp_layer.sport in self.suspicious_ports:
                        risk = "High"
                
                elif ICMP in packet:
                    protocol = "ICMP"
                    risk = "Medium"  # ICMP can be used for reconnaissance
                
                # Add to packet display
                self.window.after(0, lambda ts=timestamp, si=source_ip, di=dest_ip, p=protocol, po=port, f=flags, r=risk: 
                                self.display_packet(ts, si, di, p, po, f, r))
                ...
                if risk == "High":
                    ...
                    self.suspicious_packets.append(suspicious_packet)
                    msg = f"🚨 SUSPICIOUS PACKET: {source_ip} -> {dest_ip}:{port} ({protocol})"
                    self.window.after(0, lambda m=msg: self.log_suspicious(m))
        
        except Exception as e:
            pass  # Silently ignore packet processing errors

    def display_packet(self, timestamp, src_ip, dst_ip, protocol, port, flags, risk):
        """Display packet information in the analysis tab"""
        try:
            # Limit displayed packets to prevent GUI slowdown
            if len(self.packet_tree.get_children()) > 1000:
                # Remove oldest entries
                for item in list(self.packet_tree.get_children())[:100]:
                    self.packet_tree.delete(item)
            
            risk_icon = "🔴" if risk == "High" else "🟡" if risk == "Medium" else "🟢"
            item = self.packet_tree.insert("", 0, values=(
                timestamp, src_ip, dst_ip, protocol, port, flags, f"{risk_icon} {risk}"
            ))
            
            # Color code by risk
            if risk == "High":
                self.packet_tree.item(item, tags=("high_risk",))
            elif risk == "Medium":
                self.packet_tree.item(item, tags=("medium_risk",))
            else:
                self.packet_tree.item(item, tags=("low_risk",))
