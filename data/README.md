# 🛡️ KeySentinel Pro - Advanced Anti-Keylogger Defense System
...
An optional AI chat panel is available in the Dashboard tab. It uses a Sambanova-compatible API endpoint and your Sambanova API key.

Setup steps (recommended):

- Set the Sambanova API URL and API key as environment variables in PowerShell (preferred, avoids writing secrets to disk):

```powershell
setx SAMBANOVA_API_URL "https://api.your-sambanova-endpoint.example/v1/chat"
setx SAMBANOVA_API_KEY "<your-api-key>"
```

- Alternatively you can click "Set API Key" in the Dashboard -> AI Chat panel and paste your key; it will be saved to `config/ai_config.json` in the application folder (stored in plaintext locally).

Notes:
- The module `ai_chat.py` expects `SAMBANOVA_API_URL` to point to the correct Sambanova model endpoint. Update it to match Sambanova's API docs for your account.
- The app will try to use the Python `requests` package for HTTP; install it with `pip install requests` (already included in `requirements.txt`).
## 🛠️ Development
